<?php/***********************************************************************//*                                                                     *//*  This file is created by deZender                                   *//*                                                                     *//*  deZender (Decoder for Zend Encoder/SafeGuard):                     *//*    Version:      0.9.3.1                                            *//*    Author:       qinvent.com                                        *//*    Release on:   2005.12.5                                          *//*                                                                     *//***********************************************************************/  echo '<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN"><html><head><title>HYIP Manager Pro</title><link href="images/adminstyle.css" rel="stylesheet" type="text/css"></head><body bgcolor="#FFFFF2" link="#666699" vlink="#666699" alink="#666699" leftmargin="0" topmargin="0" marginwidth="0" marginheight="0" ><center><br><br><br>	 <table cellspacing=0 cellpadding=1 border=0 width=80% he';  echo 'ight=100% bgcolor=#ff8d00>	   <tr>	     <td>           <table width="100%" height="100%" border="0" cellpadding="0" cellspacing="0">             <tr bgcolor="#FFFFFF" valign="top"> <td bgcolor=#FFFFFF>';  if (function_exists('curl_init')) {
    ...
    if (0 < filesize($outfile)) {
        ...
        print '' . 'Error: GPG can not encrypt data:<br><pre>' . $buf . '</pre>';
    }
    unlink($infile);
    unlink($outfile);
    ...
    if ($keyID == $settings['ebullion_keyID']) {
        if (is_file('' . $xmlfile)) {
            ...
            print 'Error: Can not find decripted file! It seems you have upload incorrect secting.gpg and puring.gpg';
        }
        $data = parsexml($xmlcert);
        if ($data['status'] == 'balance') {
            print 'Test status: OK<br>Your balance is ' . $data['amount'] . ' USD';
        } else {
            if ($data['status'] == 'error') {
                print 'Error: ' . $data['text'] . ': ' . $data['additional'];
            } else {
                print 'Unknown Error!';
            }
        }
